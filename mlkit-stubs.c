...
uintptr_t c_jwt_valid_new(Context ctx, long alg, uintptr_t exn)
{
  jwt_valid_t *jwt = NULL;
  alg = convertIntToC(alg);
  int ret = jwt_valid_new(&jwt, alg);
  if (ret != 0 || jwt == NULL)
  {
    raise_exn(ctx, exn);
    return 0;
...
uintptr_t c_jwt_valid_get_grant(Region strRho, Context ctx, uintptr_t jwt, String key, uintptr_t exn)
{
  char cKey[1000];
  convertStringToC(ctx, key, cKey, 1000, exn);
  const char *result = jwt_valid_get_grant((jwt_valid_t *)jwt, cKey);
  if (result == NULL)
  {
    raise_exn(ctx, exn);
    return 0;
...
long c_jwt_valid_get_grant_int(Context ctx, uintptr_t jwt, String key, uintptr_t exn)
{
  char cKey[1000];
  convertStringToC(ctx, key, cKey, 1000, exn);
  long result = jwt_valid_get_grant_int((jwt_valid_t *)jwt, cKey);
  if (errno == ENOENT)
  {
    raise_exn(ctx, exn);
    return 0;
...
int c_jwt_valid_get_grant_bool(Context ctx, uintptr_t jwt, String key, uintptr_t exn)
{
  char cKey[1000];
  convertStringToC(ctx, key, cKey, 1000, exn);
  int result = jwt_valid_get_grant_bool((jwt_valid_t *)jwt, cKey);
  if (errno == ENOENT)
  {
    raise_exn(ctx, exn);
    return 0;
...
uintptr_t c_jwt_valid_get_grants_json(Region strRho, Context ctx, uintptr_t jwt, String key, long key_len, uintptr_t exn)
{
  char *result = NULL;
  if (key == NULL)
  {
    result = jwt_valid_get_grants_json((jwt_valid_t *)jwt, NULL);
  }
  else
  {
    key_len = convertIntToC(key_len);
    char cKey[key_len + 1];
    convertStringToC(ctx, key, cKey, key_len + 1, exn);
    result = jwt_valid_get_grants_json((jwt_valid_t *)jwt, cKey);
  }
  if (result == NULL)
  {
    raise_exn(ctx, exn);
...
void c_jwt_valid_add_grant(Context ctx, uintptr_t jwt, String key, String value, uintptr_t exn)
{
  char cKey[1000];
  convertStringToC(ctx, key, cKey, 1000, exn);
  char cValue[1000];
  convertStringToC(ctx, value, cValue, 1000, exn);
  int result = jwt_valid_add_grant((jwt_valid_t *)jwt, cKey, cValue);
  if (result != 0)
  {
    raise_exn(ctx, exn);
  }
...
void c_jwt_valid_add_grant_int(Context ctx, uintptr_t jwt, String key, long value, uintptr_t exn)
{
  char cKey[1000];
  convertStringToC(ctx, key, cKey, 1000, exn);
  value = convertIntToC(value);
  int result = jwt_valid_add_grant_int((jwt_valid_t *)jwt, cKey, value);
  if (result != 0)
  {
    raise_exn(ctx, exn);
  }
...
void c_jwt_valid_add_grant_bool(Context ctx, uintptr_t jwt, String key, int value, uintptr_t exn)
{
  char cKey[1000];
  convertStringToC(ctx, key, cKey, 1000, exn);
  value = convertBoolToC(value);
  int result = jwt_valid_add_grant_bool((jwt_valid_t *)jwt, cKey, value);
  if (result != 0)
  {
    raise_exn(ctx, exn);
  }
...
void c_jwt_valid_add_grants_json(Context ctx, uintptr_t jwt, String json, long json_size, uintptr_t exn)
{
  json_size = convertIntToC(json_size);
  char cJson[json_size + 1];
  convertStringToC(ctx, json, cJson, json_size + 1, exn);
  int result = jwt_valid_add_grants_json((jwt_valid_t *)jwt, cJson);
  if (result != 0)
  {
    raise_exn(ctx, exn);
  }
...
void c_jwt_valid_del_grants(Context ctx, uintptr_t jwt, String key, uintptr_t exn)
{
  int result;
  if (key == NULL)
  {
    result = jwt_valid_del_grants((jwt_valid_t *)jwt, NULL);
  }
  else
  {
    char cKey[1000];
    convertStringToC(ctx, key, cKey, 1000, exn);
    result = jwt_valid_del_grants((jwt_valid_t *)jwt, cKey);
  }
  if (result != 0)
  {
    raise_exn(ctx, exn);
